...
如果旧图床文件已经按 URL 路径组织，可以将整个目录树只读挂载到 `/data/migrations`。ImageSilo 会把该目录下的相对路径直接作为公网路径，不写入数据库，也不要求创建历史路径映射：

```text
/data/migrations/i/2022/04/example.jpg  ->  /i/2022/04/example.jpg
```

只会公开 JPEG、PNG、WebP 和 GIF 普通文件；路径穿越、越界软链接和保留的系统路由都会被拒绝。数据库中的历史路径映射优先于迁移目录中的同名文件。

这种方式适合冷迁移或长期只读兼容。需要在后台管理、搜索、设置可见性或转换 WebP 的图片，仍应使用下方的单张导入接口。

后台“迁移管理”会递归扫描该目录并平铺展示所有可公开交付的图片，支持搜索、格式/大小筛选和复制链接。列表使用 30 分钟惰性快照：缓存过期后的首次访问才重新扫描，不会后台定时扫盘；目录有外部变动时可点击“刷新”立即重建快照。图片直链不依赖该列表快照，文件变动仍按实时文件状态交付。默认只读；若要允许永久删除专用迁移副本，挂载必须可写、容器用户 `10001:10001` 必须拥有删除权限，并显式设置 `IMAGESILO_MIGRATION_MUTATIONS=true`。删除图片后只会向上递归移除真正为空的父目录（包括空的顶层 `i` 或 `images` 目录），不会删除迁移根目录，也不会清理仍含非图片文件的目录。原始迁移快照应继续独立保留。
