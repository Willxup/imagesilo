...
export function AliasPage() {
  ...
  const create = useMutation({
    ...
    onError: () => { setUploadProgress(0); toast.error(t('aliases.createFailed'), { id: 'alias-create' }) },
  })
  ...
  return (
    <section>
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-2">
        <form ref={createFormRef} onSubmit={createAlias}>
          <ComponentCard className="h-full" title={t('aliases.create')}>
            <label className="block font-medium" htmlFor="alias-path">{t('aliases.path')}</label>
            ...
            <label className="alias-file-picker mt-1.5">
              <span className="alias-file-picker-icon"><Icon name="image" /></span>
              ...
              <Input className="sr-only" id="alias-image" name="image" type="file" aria-label={t('aliases.imageFile')} accept="image/jpeg,image/png,image/webp,image/gif,.jpg,.jpeg,.png,.webp,.gif" required onChange={(event) => setSelectedFile(event.target.files?.[0] ?? null)} />
            </label>
            <Button className="standard-action-button mt-6" type="submit" disabled={create.isPending || !selectedFile}><Icon name={create.isPending ? 'loader' : 'plus'} className={create.isPending ? 'animate-spin' : ''} />{create.isPending ? `${t('aliases.saving')} ${uploadProgress}%` : t('aliases.create')}</Button>
            {created ? <p className="mt-4 text-green" aria-live="polite">{t('aliases.created', { path: created.path })}</p> : null}
        ...
        <form onSubmit={resolveAlias}>
          <ComponentCard className="h-full" title={t('aliases.resolve')}>
            <label className="block font-medium" htmlFor="resolve-alias-path">{t('aliases.path')}</label>
            <Input className="mt-1.5" id="resolve-alias-path" name="resolvePath" required maxLength={2048} />
            <Button className="mt-6" variant="outline" type="submit" disabled={resolve.isPending}><Icon name="search" />{resolve.isPending ? t('common.working') : t('aliases.resolve')}</Button>
            {resolved ? <div className="mt-5 rounded-xl bg-canvas p-4"><code className="break-all">{resolved.path}</code><p className="mt-2 text-sm text-muted-foreground"><a className="text-primary hover:underline" href={`/image/${resolved.imageId}`} target="_blank" rel="noreferrer">{t('aliases.target')}</a></p></div> : null}
      ...
      <ComponentCard className="mt-6" title={t('aliases.existing')}>
        ...
        {query.data?.items.length === 0 ? <p className="mt-4 text-muted-foreground">{t('aliases.empty')}</p> : null}
        <ul className="mt-4 grid gap-3">
          {query.data?.items.map((alias) => (
            <li className="flex flex-col gap-3 rounded-xl border border-line bg-canvas/60 p-4 sm:flex-row sm:items-center sm:justify-between" key={alias.id}>
              <div className="min-w-0"><code className="break-all text-sm">{alias.path}</code><p className="mt-1 text-xs text-muted-foreground">{alias.source}</p></div>
              <div className="flex shrink-0 gap-2">
                <Button size="xs" variant="outline" type="button" onClick={() => void copyText(new URL(alias.path, window.location.origin).toString()).then(() => toast.success(t('toast.linkCopiedSimple'))).catch(() => toast.error(t('toast.copyFailed')))}><Icon name="copy" />{t('common.copy')}</Button>
                <Button size="xs" variant="destructive" type="button" disabled={remove.isPending} onClick={() => setDeleteTarget(alias)}><Icon name="trash" />{t('common.delete')}</Button>
              </div>
            </li>
          ))}
        </ul>
      ...
      <ConfirmDialog open={Boolean(deleteTarget)} onOpenChange={(open) => !open && setDeleteTarget(null)} title={t('aliases.deleteTitle')} description={t('aliases.confirmDelete', { path: deleteTarget?.path ?? '' })} confirmLabel={t('common.delete')} cancelLabel={t('common.cancel')} closeLabel={t('common.close')} destructive pending={remove.isPending} onConfirm={() => deleteTarget && remove.mutate(deleteTarget.id)} />
    </section>
  )
}
